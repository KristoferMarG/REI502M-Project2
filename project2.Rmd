...
We decided to try 3 different ways of discretizising our numerical variables. We used equal frequency, equal intervals an k-means clustering. In our opinion it was not necessary to remove / ignore any attributes because each of the attributes was a specific medical test and therefore had some valuable information. We tried a few different values for the number of bins in the discretization but it didn't seem to change that much for our data so we decided on having 10 bins.
...
The mean for confidence and lift are pretty similar in all three cases but since the rule set that we get with the equal frequency discretization has a considerably higher support and the set is much smaller we decide to use that one.
...
inspect(head(freq.rules, n = 200, by ="lift"))
#inspect(head(freq.rules, n = 30, by ="confidence"))
#inspect(head(freq.rules, n = 30, by ="support"))

inspect(head(interval.rules, n = 100, by ="lift"))
#inspect(head(cluster.rules, n = 30, by = "lift"))
